-- Run this FIRST to add the self-improvement system tables
...
CREATE TABLE IF NOT EXISTS public.user_test_assignments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_conversation_outcomes_satisfaction ON public.conversation_outcomes(satisfaction_score);
CREATE INDEX IF NOT EXISTS idx_learning_patterns_type ON public.learning_patterns(pattern_type);
CREATE INDEX IF NOT EXISTS idx_dynamic_prompts_active ON public.dynamic_prompts(is_active, prompt_type);
